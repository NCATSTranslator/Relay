...
class AppConfig(ARSAppConfig):
    name = 'tr_ara_shepherd_aragorn.shepherd_aragorn_app' # must be dot path for module
    actors = [make_actorconf('infores:shepherd-aragorn', 'runquery', ['general', 'workflow'],
                             'asyncquery')] # tuple of remote, name, channel
    app_path = 'ara-shepherd-aragorn'
    regex_path = '^' + app_path + '/'

...
NCATS_URL = 'https://transltr.io:7073/query'

def index(req):
    return HttpResponse('NCATS ARA wrapper API available via POST at %s\n'
                        % req.build_absolute_uri(
                            reverse('ara-ncats-runquery')))

def ncats(data, timeout=600):
    r = requests.post(NCATS_URL, json=data, timeout=timeout)

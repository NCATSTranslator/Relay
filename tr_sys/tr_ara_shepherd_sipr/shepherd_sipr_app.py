...
class AppConfig(ARSAppConfig):
    name = 'tr_ara_shepherd_sipr.shepherd_sipr_app' # must be dot path for module
    actors = [make_actorconf('infores:shepherd-sipr', 'runquery', ['general', 'workflow'],
                            'asyncquery')] # tuple of remote, name, channel
    app_path = 'ara-shepherd-sipr'
    regex_path = '^' + app_path + '/'

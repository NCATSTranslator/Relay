...
class AppConfig(ARSAppConfig):
    name = 'tr_ara_shepherd_bte.shepherd_bte_app' # must be dot path for module
    actors = [make_actorconf('infores:shepherd-bte', 'runquery', ['general', 'workflow'],
                             'asyncquery')] # tuple of remote, name, channel
    app_path = 'ara-shepherd-bte'
    regex_path = '^' + app_path + '/'
